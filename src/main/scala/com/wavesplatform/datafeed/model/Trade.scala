...
object Trade {
  ...
  private def setTrade(amountAsset: Option[String], priceAsset: Option[String], timestamp: Long, id: String, order1ts: Long, order2ts: Long, price: Long, amount: Long, buyer: String, seller: String, matcher: String): Trade =
    apply(amountAsset.getOrElse("DCC"), priceAsset.getOrElse("DCC"), timestamp, id, (if(order1ts > order2ts) 0.toByte else 1.toByte), price, amount, WavesAddress.fromPublicKey(buyer), WavesAddress.fromPublicKey(seller), matcher)
